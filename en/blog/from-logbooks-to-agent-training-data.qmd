...
Here's the reframe that makes this possible: every entry in a farming logbook is already, structurally, an **observation, reasoning, action, worst alternative, consequence** record. Cloudy, no wind, post-bloom (observation) → *because the forecast shows rain tomorrow and the window will close* (reasoning) → applied ureum, borium, zinc (action) → *the worst choice here would have been a heavy copper spray that would have burned the leaf surface in the cold* (worst alternative) → maximal leaf absorption without evaporation (consequence). That structure is not accidental. It's how any experienced practitioner documents a decision they want a successor to trust and repeat. And it's also, not coincidentally, richer than the simple state-action-reward loop that reinforcement learning is built around. The reasoning dimension is what Chain-of-Thought training requires; the worst-alternative dimension is what DPO and process reward models need. The format a farmer used to teach a human apprentice contains everything needed to teach an AI agent to reason, not just to copy.
...
- [There Is No Data Like Strange Data](there-is-no-data-like-strange-data.qmd) — why the exceptional entries in a logbook are worth more than all the routine ones combined
